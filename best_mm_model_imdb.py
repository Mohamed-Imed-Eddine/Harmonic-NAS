...
parser.add_argument('--config-file', default=directory+'/configs/search_config_imdb.yml')
...
def eval_worker(args):
    ...
    [5,5,5,3,7,7,7,3,7,7,5,7,5,3,5,7,7,5,7,5],
    [6,4,4,3,4,4,3,6,4,3,3,4,6,3,4,3,6,4,4,6],
    [4,2,3,2,3],
    )
    ...
    print("Image Backbone: F1-W: {:.3f}%, Latency: {:.3f}ms, Energy: {:.3f}mJ".format(acc1, Lat1, Enrg1))
    print("Text Backbone: F1-W: {:.3f}%, Latency: 1.09ms, Energy: 1.40mJ".format(acc2))
    ...
    subnet1_channels = []
    out = image_subnet(torch.randn(1, 3, 224, 224).cuda(args.gpu))
