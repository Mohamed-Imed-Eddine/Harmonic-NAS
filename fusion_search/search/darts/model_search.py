...
class FusionCell(nn.Module):
    def __init__(self, steps, node_steps, multiplier, args):
        ...
        self._initialize_step_nodes(args, node_steps=node_steps)

    def _initialize_step_nodes(self, args, node_steps):
        for i in range(self._steps):
            num_input = self.num_input_nodes + i
            # step_node = AttentionSumNode(args, num_input)
            step_node = FusionNode(node_steps, args.node_multiplier, args)
...
class FusionNetwork(nn.Module):
    ...
    def __init__(self, steps, node_steps, multiplier, num_input_nodes, num_keep_edges, args, 
        ...
        self.cell = FusionCell(steps, node_steps, multiplier, args)
    ...
    def forward(self, input_features):
        # print("_num_input_nodes size :", self._num_input_nodes)
        # print("input_features size :", len(input_features))
        assert self._num_input_nodes == len(input_features)
        weights = F.softmax(self.alphas_edges, dim=-1)
        # print(f"alphas edges in forward :{self.alphas_edges}")
    ...
    def _initialize_alphas(self):
        ...
        # print("initialize alphas edges")
        # print("k :", k)
        # print("num_ops :", num_ops)
        # print("alphas_edges :", self.alphas_edges.shape)
