...
from backbones.ofa.imagenet_classification.elastic_nn.modules.dynamic_layers import (
    DynamicMBConvLayer,
)
from backbones.ofa.utils.layers import (
    ConvLayer,
    IdentityLayer,
    LinearLayer,
    MBConvLayer,
...
from backbones.ofa.imagenet_classification.networks import MobileNetV3
from backbones.ofa.utils import make_divisible, int2list, val2list, MyNetwork
...
#             in_channels, input_channel, kernel_size=3, stride=2, act_func="h_swish"
#         )
...
#     def load_weights_from_pretrained_supernet(self, checkpoint_path): # --> checked
...
#     def mutate_and_reset(self, cfg, mutate_prob=0.1):
...
#                 r = random.random()
#                 if r < mutate_prob:
#                     cfg[k][_i] = pick_another(cfg[k][_i], int2list(self.cfg_candidates[k]))
...
#     def crossover_and_reset(self, cfg1, cfg2, crx_prob=0.5):
#         def _cross_helper(g1, g2, crx_prob):
#             assert type(g1) == type(g2)
#             if isinstance(g1, int):
#                 return g1 if random.random() < crx_prob else g2
#             elif isinstance(g1, list):
#                 return [v1 if random.random() < crx_prob else v2 for v1, v2 in zip(g1, g2)]
#             else:
#                 raise NotImplementedError

#         cfg = {}
#         for k in ['ks', 'e', 'd']:
#             cfg[k] = _cross_helper(cfg1[k], cfg2[k], crx_prob)
...
#     def init_population(self, n_samples=None): # -> checked
        
#         samples = [] 
#         for _ in range(n_samples):
#             samples.append(self.sample_active_subnet())
...
#         _subnet = MobileNetV3(
#             first_conv, blocks, final_expand_layer, feature_mix_layer, classifier, self.resolution
#         )
#         _subnet.set_bn_param(**self.get_bn_param())
#         return _subnet
...
#     def crossover_and_reset1(self, cfg1, cfg2, crx_prob=0.5):
#         def _cross_helper(g1, g2, crx_prob):
#             assert type(g1) == type(g2)
#             if isinstance(g1, int):
#                 return g1 if random.random() < crx_prob else g2
...
#         for k in ['ks', 'e', 'd']:
#             cfg[k] = _cross_helper(cfg1[k], cfg2[k], crx_prob)
#         # assert cfg1['net_id'] == cfg2['net_id']
#         cfg['net_id'] = cfg1['net_id']
#         self.set_active_subnet(
#             cfg['ks'], cfg['e'], cfg['d']
#         )
#         return cfg
...
class OFAMobileNetV3(MobileNetV3):
    def __init__(
        ...
        if self._type == 'avmnist':
            base_stage_width = [16, 16, 24, 40, 80] 
            stride_stages = [1, 2, 2, 2]
            act_stages = ["relu", "relu", "relu", "h_swish", "h_swish", "h_swish"]
            se_stages = [False, False, True, False, True, True]
        
        elif self._type == 'imdb':
        ...
        # first conv layer
        first_conv = ConvLayer(
            self.in_ch, input_channel, kernel_size=3, stride=2, act_func="h_swish"
        )
        first_block_conv = MBConvLayer(
            in_channels=input_channel,
            out_channels=first_block_dim,
    ...
    def crossover_and_reset1(self, cfg1, cfg2, crx_prob=0.5, backbone_num=''):
        def _cross_helper(g1, g2, crx_prob):
            assert type(g1) == type(g2)
            if isinstance(g1, int):
                return g1 if random.random() < crx_prob else g2
        ...
        for k in ['ks', 'e', 'd']:
            cfg[k] = _cross_helper(cfg1[k], cfg2[k], crx_prob)
        # assert cfg1['net_id'] == cfg2['net_id']
        cfg['net_id'+backbone_num] = cfg1['net_id'+backbone_num]
        self.set_active_subnet(
            cfg['ks'], cfg['e'], cfg['d']
        )
        return cfg
