...
parser.add_argument("--net", metavar="OFANET", default= "ofa_mbv3_d234_e346_k357_w1.2", help="OFA networks")
...
def eval_worker(gpu, ngpus_per_node, args):
    ...
    # Build the supernet
    supernet = ofa_net(args.net, pretrained=False) 
    supernet.classifier.linear = torch.nn.Linear(in_features=supernet.classifier.linear.in_features, out_features=args.n_classes, bias=True)
    ...
    # Load LUT for latency/energy characterization on the edge device
    ...
    with open(args.maxout_weights, 'rb') as f:
        ...
        textnet.load_state_dict(checkpoint)
    ...
    if(args.resume_evo == 0): # First population (evolution)
        ...
        args.start_evo = 0
        # save_ooe_population(directory, 0, parent_ooe_popu, 'imdb')
    else:
        print('resuming...') # To resume the search from an already saved population
        f = open(directory+'/results/imdb/popu/resume_'+str(args.start_evo)+'.popu', 'rb')   
    ...
    for evo_outer in range(args.start_evo, args.evo_search_outer.evo_iter):
        ...
        if(args.rank == 0):
            ...
            print("###################################################")
            save_results(directory, evo_outer, 'Init_B1', backbones, 'imdb')
        ...
        backbones = RankAndCrowdingSurvival_Inner_Acc(backbones, normalize=None, 
                                                    n_survive=math.ceil(args.evo_search_outer.parent_popu_size*args.evo_search_inner.survival_ratio), perfo_metric='F1-W@1')
        ...
        if(args.rank==0):
            ...
            print("###################################################")
            save_results(directory, evo_outer, 'Elites_B1', backbones, 'imdb')
        ...
        if(args.rank==0):
            ...
            print("###################################################")
            save_results(directory, evo_outer, 'Fusion_Popu', parent_ooe_popu, 'imdb')


        save_ooe_population(directory, evo_outer, parent_ooe_popu, 'imdb')
        ...
        eval_results = subnets_nas_eval.imdb_fusion_validate(
            my_subnets_to_be_evaluated,
            train_loader,
            test_loader,
            supernet,
        ...
        f = open(directory+'/results/imdb/popu/evo_'+str(evo_outer)+'.popu', 'rb')   
        ...
        for i, row in enumerate(actual_popu, start=0):
            ...
            actual_popu[i]['F1-W@1'] = eval_results[i]['F1-W@1']
        ...
        if(args.rank == 0):
            ...
            print("###################################################")
            save_results(directory, evo_outer, 'Fusion_Popu_Results', actual_popu, 'imdb')        
        ...
        survivals_ooe = RankAndCrowdingSurvival_Outer_Acc(pop=actual_popu, normalize=None, 
                                                    n_survive=n_survive, perfo_metric='F1-W@1')
        ...
        if args.rank == 0:
            ...
            print("###################################################")
            save_results(directory, evo_outer, 'Elites_MM', survivals_ooe, 'imdb')
        ...
        save_resume_population(directory, evo_outer+1, parent_ooe_popu, 'imdb')
